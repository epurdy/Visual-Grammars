...
let calc_bounds gram filt =
  let size = 60 in
  let topids = Filtration.top_level_ids filt.levels.(filt.depth-1) in  
  ...
  in
	...
	match prod.cdata.geom with 
	  ...
	  | Watson watson ->
	      ...
	      let unpack = Watson.watson_distro_family.Distro.unpack watson in
	      let pqr_shape = unpack.Watson.mean_shape in
