...
doit('./sdf_tuning.native -example %s -sdf %s -niters %d %s' % (
      example, sdf, niters, ' '.join(training)))

for i in xrange(niters + 1):
  show_grammar(dir, 'gram.%d.d' % i, 'sdf_tuning.%d.gram' % i)
#   doit('mkdir -p %s/gram.%d.d' % (dir,i))
#   doit('rm -rf %s/gram.%d.d/*' % (dir,i))
#   doit(('./show_grammar.native -gramfile tmp/sdf_tuning.%d.gram ' +
