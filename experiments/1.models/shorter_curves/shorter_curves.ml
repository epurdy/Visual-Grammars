...
module Svg = Visualization.Svg
module Viz = Visualization.Viz
...
let do_parse incurve_name excurve_name sdf_name out_name sdf_out_name = 
  ...
  let exsdf = 
    if sdf_name = "x" then
      Sdf.make_sparse_family (Array.length excurve) 2 
    else
      bottom_out_family (Sdf.load_family sdf_name)
  in 

  let _ = 
    Sdf.save_family exsdf "tmp/shorter.sdf";
    doit (sprintf "./show_sdf.native -sdf tmp/shorter.sdf -curve %s -fname %s" excurve_name sdf_out_name);
  in
...
let _ =
  let incurve_name, excurve_name, sdf_name, out_name, sdf_out_name = 
    try
     Sys.argv.(1), Sys.argv.(2), Sys.argv.(3), Sys.argv.(4), Sys.argv.(5)
    with _ ->
      printf "usage: ./%s in.curve ex.curve ex.sdf out.svg sdf.svg" Sys.argv.(0);
      "", "", "", "", ""
  in
    do_parse incurve_name excurve_name sdf_name out_name sdf_out_name
