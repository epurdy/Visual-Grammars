...
(*   let im = Pnm.load_pgm "romer/clean/IMG0200.PGM" in *)
  ...
  let smooth2 = Filter.smooth Image.float_ops fim 1.0 in
...
(* notes
 ...
 - declare largest segmment to be background, give it z=0, everything
   else z=1

 - go from segmentation to oriented edge map by marking a pixel as an
   edge pixel if it is on the border between two segment with
   different z-values and belongs to the segment with higher z-value

 - pick orientations so that edges go clockwise around things with
   high z-value.

 - for each segment, try either increasing or decreasing its z-index
   and redoing the global parse

 - do best-first search over this space.
