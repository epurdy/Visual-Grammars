...
module Segmentation = Detection.Segmentation
...
let infer_everything params =
  compute segmentation;
  (* don't do cleanup until subsequent steps work, there are a lot of
     nasty edge-cases that cleanup might hide from us *)
  clean up segmentation;(* use ising model with softmax, inv weighted by intensity difference *)
  make region graph; (* just iterate over labelled seg image, keep track of neighbors from diff segs  *)
  draw region graph; (* calculate centroids of each region, draw and then connect neighbors *)
  make boundary graph; 
  (* taking planar dual is dumb?

     can't get planar dual without plane embedding, which we can't nec. get
     super easily

     color in pixels that have at least two diff regions as 8 nbrs (4 nbrs???)
     but some of those don't really deserve to be vertices...

     being a local max for # of diff regions that are your neighbors?
     
 *)
  

  let infer_everything params edgedata smim smim2 segments name =
    ...
    let _ = Pnm.save_pgm (Image.map (fun x -> round x) smim) "helmholtz/smim.pgm" in
    let _ = Pnm.save_pgm (Image.map (fun x -> round x) smim2) "helmholtz/smim2.pgm" in
	...
	let _ = printf "doing segmentation\n%!" in
	let labelimage = Segmentation.get_segmentation smim2 (local.w,local.h) curl in
	...
	let zim = Image.map (fun x -> round (255. *. (logistic (x*.0.5)))) local.zindex in
      ...
      let segim = Segmentation.draw_segmentation labelimage in
	...
	Pnm.save_ppm segim (sprintf "%s.%d.seg.pgm" name foo);
    done;
...
(*   in *)
  ...
  let im = Pnm.load_pgm "small-asl-1.pgm" in
(*   let im = Pnm.load_pgm "romer/clean/IMG0200.PGM" in *)
  let fim = Image.map foi im in
  let smooth = Filter.smooth Image.float_ops fim 4.0 in
  let smooth2 = Filter.smooth Image.float_ops fim 0.8 in
  ...
  let local = infer_everything params edgedata smooth smooth2 segs "helmholtz/z" in
